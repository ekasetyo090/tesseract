...
with st.container(border=None):
    ...
    with col2:
        ...
        fig, ax = plt.subplots()
        plot = sns.lineplot(data=df_filter,x='start_time(UTC)',
...
with st.container(border=None):
    if not df_video['hour'].empty: 
        ...
        with col2:
            fig, ax = plt.subplots()
            sns.lineplot(
                data=df_filter, x="hour", y=metrics.get(option_hour),ax=ax)
...
with st.container(border=None):
    if not df_video['duration(s)'].empty: 
        ...
        with col2:
            fig, ax = plt.subplots()
            sns.regplot(data=df_filter, x="duration(s)", y=metrics.get(option_duration),
                        ax=ax,logx=True)
